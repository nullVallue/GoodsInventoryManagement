...
class EditAddSupplierPage extends StatefulWidget {
  final Map<String, dynamic> productMap;
  const EditAddSupplierPage({super.key, required this.productMap});
  

  @override
  State<EditAddSupplierPage> createState() => _EditAddSupplierPageState();
}

class _EditAddSupplierPageState extends State<EditAddSupplierPage>{
  ...
  Future<void> loadDropdown() async {
    supplierList = await DatabaseMethods.getUnusedSupplierList(widget.productMap['ProductID']);
  }

  addProductSupplier(String supplierEmail, String supplierName) async {
    showLoadingDialog(
    ...
    Map<String, dynamic> productSupplierMap = {
      'ProductID' : widget.productMap['ProductID'],
      'SupplierID' : 0,
      'ProductSupplierID' : 0,
    };
    ...
    if(ddlVisible){
      ...
      int supplierId = await DatabaseMethods.getSupplierIdByName(supplierName);

      if(supplierId == 0){
        alertMsg = 'Failed to fetch supplier ID';
      }
      else{
        productSupplierMap['SupplierID'] = supplierId;
        alertMsg = await DatabaseMethods.addProductSupplierDetails(productSupplierMap);
      }
  ...
  createSuccess(BuildContext oriContext){
    ...
    showAlertDialog(
      ...
      message: "Supplier has been added to product successfully",
      context: context,
      bgColor: Theme.of(context).colorScheme.background,
      titleColor: Theme.of(context).colorScheme.onBackground,
      msgColor: Theme.of(context).colorScheme.onTertiaryContainer,
      ...
      btnOnPressed: () => {
        ...
        navigateTo(oriContext, widget, ViewProductSupplierPage(productMap: widget.productMap), animationType: "scrollLeft"),
        // Navigator.push(
        //   context,
...
// section 1
                    Container(
                      ...
                      Row(
                        children: <Widget>[
                        BackButton(
                          onPressed: ()=> navigateTo(context, widget, ViewProductSupplierPage(productMap: widget.productMap), animationType : 'scrollLeft'),
                        )
                        ]
                      ),
                    ),
                                  ...
                                  onPressed: () => {
                                    if(ddlVisible){
                                      ...
                                      if(_existSupplierFormKey.currentState!.validate()){
                                        addProductSupplier('', _dropdownSupplierValue??''),
                                      }
                                    }
                                    else{
                                      ...
                                      if(_supplierFormKey.currentState!.validate() && _nameFormKey.currentState!.validate()){
                                        ...
                                        addProductSupplier(_supplierController.text, _nameController.text)
                                      }
                                    },
                                  },
                                  ),                               
